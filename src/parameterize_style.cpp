...
static void parameterize_map_language(mapnik::Map &m, char * parameter)
{
	...
	name_replace[0] = 0;
	syslog(LOG_DEBUG, "DEBUG: Internationalizing map to language parameter: %s", parameter);
	tok = strtok(data, ",");
...
{
	syslog(LOG_INFO, "INFO: Loading parameterization function for %s", function_name);
	...
	if (strcmp(function_name, "") == 0) {
		...
		syslog(LOG_WARNING, "WARNING: unknown parameterization function for %s", function_name);
	}
