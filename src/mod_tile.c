...
static int tile_handler_metrics(request_rec *r)
{
	...
	ap_rprintf(r, "# HELP modtile_cached_tiles_total Tiles served from the tile cache\n");
	ap_rprintf(r, "# TYPE modtile_cached_tiles_total counter\n");
	ap_rprintf(r, "modtile_cached_tiles_total{age=\"fresh\",rendered=\"no\"} %li\n", local_stats.noFreshCache);
	ap_rprintf(r, "modtile_cached_tiles_total{age=\"old\",rendered=\"no\"} %li\n", local_stats.noOldCache);
	ap_rprintf(r, "modtile_cached_tiles_total{age=\"outdated\",rendered=\"no\"} %li\n", local_stats.noVeryOldCache);
	ap_rprintf(r, "modtile_cached_tiles_total{age=\"fresh\",rendered=\"yes\"} %li\n", local_stats.noFreshRender);
	ap_rprintf(r, "modtile_cached_tiles_total{age=\"old\",rendered=\"attempted\"} %li\n", local_stats.noOldRender);
	ap_rprintf(r, "modtile_cached_tiles_total{age=\"outdated\",rendered=\"attempted\"} %li\n", local_stats.noVeryOldRender);

	ap_rprintf(r, "# HELP modtile_tiles_reponses_total Tiles served by zoom level\n");
	ap_rprintf(r, "# TYPE modtile_tiles_reponses_total counter\n");

	for (i = 0; i <= global_max_zoom; i++) {
		ap_rprintf(r, "modtile_tiles_responses_total{zoom=\"%02i\"} %li\n", i, local_stats.noRespZoom[i]);
	}
